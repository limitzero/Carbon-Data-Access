# help from http://linkpass.net/content/?id=15738&amp;from-0-to-doing-solution-builds-w-psakepart-1-of-n 
# on creating the build script 
...
#-------------------------------------------------------------------------------------------------
# Function to call xUnit for running the unit tests against an assembly
#-------------------------------------------------------------------------------------------------
function Invoke-Xunit
{
	[CmdletBinding()]
	param(
		[Parameter(Position=0,Mandatory=1)] [string]$TestDllPath = $null,
        [Parameter(Position=1,Mandatory=1)] [string]$TestDll = $null,
        [Parameter(Position=2,Mandatory=1)] [string]$ResultTrx = $null
		)
	
	Write-Host "Running tests for" $TestName "Located @" $TestDllPath "\" $TestDll " Output to:" $ResultTrx	
	exec "$tools_dir\xUnit\xunit.console.x86.exe" "$TestDll"	
}